...
parser.add_argument("--image_dir", type=str, default='./save_images/syn/model2',help="path to Dataset")
parser.add_argument("--data_root", type=str, default='./dataset/syn',help="path to Dataset")
parser.add_argument('--snapshots', type=int, default=5, help='Snapshots')
parser.add_argument('--crop', action='store_true')
parser.add_argument('--crop_size', type=int, default=500, help='Takes effect when using --crop ')
...
seg_net.load_state_dict(torch.load(checkpoint))
...
def generator_train(img):
    # pred,out = unet(img)
    pred,out = seg_net(img)  #[1,128,64,64]->[1,3,1,1],[1,19,360,360]
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for i, (images,targets,image_name) in enumerate(train_loader):
        ...
        save_image(torch.cat([images, coarse_pred, refined_pred, targets], dim=0), './predict/train.jpg')
        # Pixel-wise loss
    ...
    if epoch % (opt.snapshots) == 0:
        ...
        with torch.no_grad():
            for i, (inputs, targets, image_name) in enumerate(test_loader):
                ...
                ssims.append(ssim1)
                psnrs.append(psnr1)
                lpips_score.append(lpips1)
            ssim_eval = np.mean(ssims)
            psnr_eval = np.mean(psnrs)
            lpips_eval = np.mean(lpips_score)
