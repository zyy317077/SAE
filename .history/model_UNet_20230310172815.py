...
class CAS(nn.Module):
    ...
    def __init__(self, channels,kernel_size, reduction,bias, bn, is_up=False):
        super(CAS, self).__init__()
        
        # context extraction
        # self.context = CA(channels)
      
        self.context = DAB(channels,3,16,True,False)
        # downsampling
...
class DAB(nn.Module):
    def __init__(self, channels, kernel_size, reduction,bias=True, bn=False):
        super(DAB, self).__init__()
        self.conv = nn.Conv2d(channels, channels, kernel_size,padding=(kernel_size//2), bias=bias)
        modules_body = []
        for i in range(2):
