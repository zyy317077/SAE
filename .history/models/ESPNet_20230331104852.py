...
class ESPNet(nn.Module):
    ...
    def forward(self, input):#[1,3,512,512]
        ...
        :return: transformed feature map
        '''
        output0 = self.modules[0](input)#[1,3,256,256]
        inp1 = self.modules[1](input)#[1,3,256,256]
        inp2 = self.modules[2](input)#[1,3,128,128]

        output0_cat = self.modules[3](torch.cat([output0, inp1], 1))#[1,19,256,256]
        output1_0 = self.modules[4](output0_cat)  # down-sampled#[1,64,128,128]

        for i, layer in enumerate(self.modules[5]):
            if i == 0:
                output1 = layer(output1_0)#[1,64,128,128]
            else:
                output1 = layer(output1)#[1,64,128,128]

        output1_cat = self.modules[6](torch.cat([output1, output1_0, inp2], 1))#[1,131,128,128]


        output2_0 = self.modules[7](output1_cat)  # down-sampled#[1,128,64,64]

        for i, layer in enumerate(self.modules[8]):
            if i == 0:
                output2 = layer(output2_0)
            else:
                output2 = layer(output2)#[1,128,64,64]

        output2_cat = self.modules[9](torch.cat([output2_0, output2], 1)) # concatenate for feature map width expansion#[1,256,64,64]
        #融合
        out = self.dc(output2_cat)
        output2_c = self.up_l3(self.br(self.modules[10](output2_cat))) #RUM #[1,19,128,128]

        output1_C = self.level3_C(output1_cat) # project to C-dimensional space #[1,19,128,128] 
        comb_l2_l3 = self.up_l2(self.combine_l2_l3(torch.cat([output1_C, output2_c], 1))) #RUM #[1,19,256,256]

        concat_features = self.conv(torch.cat([comb_l2_l3, output0_cat], 1))#[1,19,256,256]

        classifier = self.classifier(concat_features)#[1,19,512,512]
        return  out,classifier

if __name__ == '__main__':
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    x = torch.randn(1,3,512,512)
    net = ESPNet(19,2,3,None)
    out,classifier= net(x)
    print(out.shape,classifier.shape)