...
class LadderBlock(nn.Module):
    ...
    def forward(self, x):#[1,256,16,16]#[1,128,32,32]#[1,64,64,64]
        out = self.inconv(x[-1])#[1,64,64,64]

        down_out = []
        # down branch
        for i in range(0,self.layers-1):
            out = out + x[-i-1]#[1,64,64,64]
            out = self.down_module_list[i](out)#[1,64,64,64]
            down_out.append(out)
...
def create_layer_basic(in_chan, out_chan, bnum, stride=1):
    layers = [BBlock(in_chan, out_chan, stride=stride)]
    for i in range(bnum-1):
        layers.append(BBlock(out_chan, out_chan, stride=1))
