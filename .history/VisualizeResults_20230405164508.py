...
def evaluateModel(args, model, test_name, img):
    ...
    img_tensor = torch.unsqueeze(img, 0)#[1,3,512,512] 
    _,img_out = model(img_tensor, None, None, True)#[1,19,512,512]
    ...
    if args.colored:
        ...
        cv2.imwrite(args.savedir + os.sep + 'c_' + test_name.replace(args.img_extn, 'png'), classMap_numpy_color)
...
def main(args):
    ...
    with torch.no_grad():
        ...
        for i in range(len(test_imgs_dir)):
            ...
            test_img = Image.open(test_imgs).convert('RGB') #(960,720)
            ...
            test_img = tfs.ToTensor()(test_img) #(3,512,512)
            ...
            evaluateModel(args, net, test_name, test_img)
