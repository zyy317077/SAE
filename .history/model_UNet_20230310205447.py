...
class DAB(nn.Module):
    ...
    def forward(self, x): #[1,32,512,512]
        # res = self.body(x)  
        res = x               #[1,32,512,512]
        sa_branch = self.SA(res) #[1,32,508,508]
        ca_branch = self.CA(res) #[1,32,1,1]
...
class spatial_attn_layer(nn.Module):
    ...
    def forward(self, x):  #[1,32,512,512]         
        x_compress = self.compress(x)  ##[1,2,512,512]
        x_out = self.spatial(x_compress) #[1,1,512,512]
        scale = torch.sigmoid(x_out)    #[1,1,512,512]
        return x * scale
