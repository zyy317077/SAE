...
class AtrousBlockPad2(nn.Module):
    ...
    def forward(self, input):
        ...
        x1 = self.atrous_layers[0](input) #[1,256,16,16]
        x2 = self.atrous_layers[1](input) #[1,256,16,16]
        x3 = self.atrous_layers[2](input) #[1,256,16,16]
        x4 = self.atrous_layers[3](input)

        x = torch.cat((x1, x2, x3, x4), 1)    #[1,768,16,16]
        x5 = self.conv1(x) 
