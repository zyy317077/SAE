...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        # Model inputs
        real_A = Variable(batch["A_input"].type(Tensor))
        real_B = Variable(batch["B_exptC"].type(Tensor))#还是取的同一个数据集中正常光图像，只不过不是同一张图像对应的
        real_C = Variable(batch["A_exptC"].type(Tensor))
        save_image(torch.cat([real_A, real_B ,real_C], dim=0), './out/111.jpg')
        ...
        if i % opt.n_critic == 0:
            ...
            pred_fake = discriminator(fake_B)
            # Pixel-wise loss
            loss_pixel = criterion_pixelwise(pred_fake, real_A)
