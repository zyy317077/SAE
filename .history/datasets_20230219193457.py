...
class ImageDataset_sRGB(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "train":
            ...
            i, j, h, w = transforms.RandomCrop.get_params(img_input, output_size=(crop_h, crop_w))#get_params允许裁剪之后的区域返回边框的坐标数值
            # img_input = TF.crop(img_input, i, j, h, w)
            # img_exptC = TF.crop(img_exptC, i, j, h, w)
            img_input = TF.resized_crop(img_input, i, j, h, w, (480,480))
            img_exptC = TF.resized_crop(img_exptC, i, j, h, w, (480,480))
...
class ImageDataset_sRGB_unpaired(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "train":
            ...
            img_input = Image.open(self.set1_input_files[index % len(self.set1_input_files)])
            img_exptC = Image.open(self.set1_expert_files[index % len(self.set1_expert_files)])
            seed = random.randint(1,len(self.set2_expert_files))#随机生成1与len(self.set2_expert_files)之间的整数
