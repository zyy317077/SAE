...
class Classifier(nn.Module):
    def __init__(self, in_channel=128, out_channel=3):
        ...
        self.model = nn.Sequential(
            ...
            *discriminator_block(in_channel, 128, normalization=True),
            *discriminator_block(128, 128, normalization=True),
            *discriminator_block(128, 128, normalization=True),
            *discriminator_block(128, 128),
            #*discriminator_block(128, 128, normalization=True),
            nn.Dropout(p=0.5),
            nn.Conv2d(128, 3, 4, padding=0),
        )
...
class ABSGN(nn.Module):
    def __init__(self, input_dim=3, dim=32):
        ...
        self.up1 = nn.Upsample(size=(480,480), mode='bilinear', align_corners=True)
        for m in self.modules():#自定义参数初始化 self.children()存储网络结构的子模块,只包括网络模块的第一代儿子模块，而self.modules()采用深度优先遍历包含网络模块的自己本身和所有的后代模块
            ...
            if classname.find('Conv2d') != -1:
                ...
                if m.bias is not None:
                    m.bias.data.zero_()
            elif classname.find('ConvTranspose2d') != -1 or classname.find("InstanceNorm2d") != -1:
                torch.nn.init.kaiming_normal_(m.weight)
                if m.bias is not None:
                    m.bias.data.zero_()
    ...
    def _Itransformer(self, out):
        ...
        return yl, yh
    def forward(self, x):
        x = self.up(x)#batch,3,256,256
        out1 = self.feat1(x)#batch*32*256*256
        DMT1_yl,DMT1_yh=self.DWT(out1)
        out2=self._transformer(DMT1_yl,DMT1_yh)#batch*128*128*128
        out2=self.feat2(out2)#batch*64*128*128
        DMT2_yl, DMT2_yh = self.DWT(out2)
        out3 = self._transformer(DMT2_yl, DMT2_yh)#batch*256*64*64
        out3 = self.feat3(out3)#batch*128*64*64
        DMT3_yl, DMT3_yh = self.DWT(out3)
        out4 = self._transformer(DMT3_yl, DMT3_yh)#batch*512*32*32
        out4 = self.feat4(out4)#batch*256*32*32
        GSA=self.GSA(out4)#batch*256*32*32
        out5=self._Itransformer(GSA)
        out5=self.IDWT(out5) #batch*64*64*64
        out6=self.feat5(out5)#batch*128*64*64
        conv = self.conv(out6)
        basic_1 = self.MGDB_1(out3)#batch*128*64*64
        out7 = torch.cat((out6,basic_1),1)#batch*256*48*48
        out8 = self.feat6(out7)#batch*256*64*64
        out9=self._Itransformer(out8)
        out9=self.IDWT(out9)#batch,32,128,128
        out10=self.feat7(out9)##batch*64*128*128
        basic_2 = self.MGDB_2(out2)##batch*64*128*128
        out11 = torch.cat((out10, basic_2),1)##batch*128*128*128
        out12 = self.feat8(out11)##batch*64*128*128
        out13 = self._Itransformer(out12)
        out14 = self.IDWT(out13)##batch*16*256*256
        out15 = self.feat9(out14)##batch*32*256*256
        out16=torch.cat((out1,out15),1)#batch*64*256*256
        out17=self.MGDB_3(out16)#batch*128*256*256
        out18=torch.cat((out16,out17),1)#batch*192*256*256
        out19=self.MGDB_4(out18)#batch*128*256*256
        out20=torch.cat((out18,out19),1)#batch*320*256*256
        out21=self.feat10(out20)#batch*128*256*256
        pred=self.feat11(out21)#batch*3*256*256
        pred=self.up1(pred)
        return conv,pred
        # return out6,conv,pred
# if __name__ == '__main__':
#     device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
#     x = torch.randn(1,3,400,703).to(device)
#     net = ABSGN(3,32).to(device)
#     conv,pred = net(x)
#     # print("x:",x.shape)
#     print("out6:",conv)
