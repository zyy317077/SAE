...
parser.add_argument("--output_dir", type=str, default="Ab/model5",help="path to save model")
parser.add_argument("--result_dir", type=str, default="result",help="path to save result")
parser.add_argument("--image_dir", type=str, default='./save_images/Ab/model5',help="path to Dataset")
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for i, (images,targets,image_name) in enumerate(train_loader):
        ...
        # loss = mse 
        loss = 0.1*mse + loss_seg + loss_ssim + loss_per
    ...
    if epoch % (opt.snapshots) == 0:
        ...
        with torch.no_grad():
            for i, (inputs, targets, image_name) in enumerate(test_loader):
                ...
                # save_image(coarse_pred, os.path.join(opt.image_dir, "%s.png" % (image_name)), nrow=1, normalize=False)
                # # 精度计算
            ...
            print(f'\npsnr:{psnr_eval:.4f}\t|ssim:{ssim_eval:.4f}\t')
