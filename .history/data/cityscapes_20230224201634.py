...
class Cityscapes(data.Dataset):
    ...
    **Parameters:**
    ...
    def __init__(self, root, train, mode1 = 'H', mode2 = 'S', target_type='semantic', transform=None):
        ...
        self.images_dir = os.path.join(self.root, 'train_L')       
        self.targets_dir = os.path.join(self.root, 'train_H')
        ...
        if not os.path.isdir(self.images_dir) or not os.path.isdir(self.targets_dir) or not os.path.isdir(self.semantics_dir):
        ...
        for file_name in os.listdir(self.images_dir):
            ...
            semantic_name = '{}_{}.png'.format(file_name.split('_L')[0],
                                              self.mode2)
                                            # self._get_target_suffix(self.mode, self.target_type))
            self.semantics.append(os.path.join(self.semantics_dir, semantic_name))
    ...
    def __getitem__(self, index):
        ...
        i, j, h, w = transforms.RandomCrop.get_params(image, output_size=(360,360))
        ...
        image,target,semantic = self.augData(image.convert('RGB'),target.convert('RGB'),semantic)
        # if self.transform:
        #     # image, target = self.transform(image, target)
        ...
        return image, target, 
    def augData(self, data, target1, target2 ):
        if self.train:
